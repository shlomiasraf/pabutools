...
from pabutools.rules.cstv import cstv_budgeting_combination
import random
...
class TestFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.donors = [
            CumulativeBallot({"A": 5, "B": 10, "C": 5}),
            CumulativeBallot({"A": 10, "B": 10, "C": 0}),
            CumulativeBallot({"A": 0, "B": 15, "C": 5}),
            CumulativeBallot({"A": 0, "B": 0, "C": 20}),
            CumulativeBallot({"A": 15, "B": 5, "C": 0}),
        ]
    ...
    def test_cstv_budgeting_with_zero_budget(self):
        ...
        for alg_str in ["ewt", "ewtc", "mt", "mtc"]:
            self.projects = Instance(
                [Project("A", 27), Project("B", 30), Project("C", 40)]
            )
            selected_projects = cstv_budgeting_combination(
                self.projects, self.donors, alg_str
            )
            self.assertEqual(
    ...
    def test_cstv_budgeting_with_budget_greater_than_max_total_needed_support(self):
        ...
        for alg_str in ["ewt", "ewtc", "mt", "mtc"]:
            ...
            self.assertEqual(
                len(selected_projects), num_projects - 1
    ...
    def test_cstv_budgeting_with_budget_between_min_and_max(self):
        for alg_str in ["ewt", "ewtc", "mt", "mtc"]:
            ...
            )  # Ensure the number of selected projects is 3 when total budget is between the minimum and maximum costs
    ...
    def test_cstv_budgeting_large_input(self):
        ...
        for alg_str in ["ewt", "ewtc", "mt", "mtc"]:
            self.projects = [
                Project(f"Project_{i}", 50) for i in range(int(num_projects / 2))
            ]
            self.projects += [
                Project(f"Project_{i+50}", 151) for i in range(int(num_projects / 2))
            ]
            self.projects = Instance(init=self.projects)
            self.donors = [
                CumulativeBallot({f"Project_{i}": 1 for i in range(num_projects)})
                for _ in range(100)
            ]
            selected_projects = cstv_budgeting_combination(
                self.projects, self.donors, alg_str
            ...
            self.assertLessEqual(
                len(selected_projects), num_projects
    ...
    def test_cstv_budgeting_large_random_input(self):
        for alg_str in ["ewt", "ewtc", "mt", "mtc"]:
            ...
            self.donors = [
                CumulativeBallot(
                    {
                        f"Project_{i}": donation
                        for i, donation in enumerate(
                            generate_donations(20, len(self.projects))
                        )
                    }
                )
                for _ in range(100)
            ...
            positive_excess = sum(
                ...
                if sum(donor.get(p.name, 0) for donor in self.donors) - p.cost >= 0
            )
            ...
            self.assertGreaterEqual(
                len(selected_projects), positive_excess
